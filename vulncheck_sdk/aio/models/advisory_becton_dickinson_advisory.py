...
class AdvisoryBectonDickinsonAdvisory(BaseModel):
    """
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in products_affected (list)
        _items = []
        if self.products_affected:
            for _item_products_affected in self.products_affected:
                _items.append(_item_products_affected.to_dict() if _item_products_affected is not None else None)
            _dict['products_affected'] = _items
        return _dict
